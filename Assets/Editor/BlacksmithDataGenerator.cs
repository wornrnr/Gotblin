...
public static class BlacksmithDataGenerator
{
    ...
    public static void GenerateAllDataAssets()
    {
        ...
        // 1. 단검 계열 생성 (공속/이속 중심, 스탯 2.3~2.5배 스케일링)
        var d1 = CreateWeaponAsset(weaponDir, "Dagger_Lv1", "dagger_1", "수습 단검", "단검", 1, 15, 0.70f, 0.20f, 5,
            new List<ItemOption> {
        ...
        var d2 = CreateWeaponAsset(weaponDir, "Dagger_Lv2", "dagger_2", "은빛 단검", "단검", 2, 35, 0.50f, 0.30f, 15,
            new List<ItemOption> {
        ...
        var d3 = CreateWeaponAsset(weaponDir, "Dagger_Lv3", "dagger_3", "그림자 단검", "단검", 3, 90, 0.30f, 0.40f, 45,
            new List<ItemOption> {
        ...
        // 2. 양손검 계열 생성 (공격력%/타겟수 중심, 스탯 2.3~2.5배 스케일링)
        var s1 = CreateWeaponAsset(weaponDir, "Sword_Lv1", "sword_1", "강철검 1단계", "양손검", 1, 30, 0.65f, 0.25f, 8,
            new List<ItemOption> {
                new ItemOption { optionType = WeaponOptionType.ATKPercent, value = 0.10f }
            });

        var s2 = CreateWeaponAsset(weaponDir, "Sword_Lv2", "sword_2", "명검 2단계", "양손검", 2, 70, 0.45f, 0.35f, 20,
            new List<ItemOption> {
        ...
        var s3 = CreateWeaponAsset(weaponDir, "Sword_Lv3", "sword_3", "용살검 3단계", "양손검", 3, 160, 0.25f, 0.45f, 60,
            new List<ItemOption> {
        ...
        // 3. 전투도끼 계열 생성 (고공격력/생명력% 중심, 스탯 2.3~2.5배 스케일링)
        var a1 = CreateWeaponAsset(weaponDir, "Axe_Lv1", "axe_1", "벌목도끼", "전투도끼", 1, 50, 0.60f, 0.30f, 10,
            new List<ItemOption> {
                new ItemOption { optionType = WeaponOptionType.HPPercent, value = 0.10f }
            });

        var a2 = CreateWeaponAsset(weaponDir, "Axe_Lv2", "axe_2", "전투도끼", "전투도끼", 2, 120, 0.40f, 0.40f, 25,
            new List<ItemOption> {
                new ItemOption { optionType = WeaponOptionType.HPPercent, value = 0.25f },
                new ItemOption { optionType = WeaponOptionType.TargetCount, value = 1f }
            });

        var a3 = CreateWeaponAsset(weaponDir, "Axe_Lv3", "axe_3", "파괴의 광도끼", "전투도끼", 3, 280, 0.20f, 0.50f, 75,
            new List<ItemOption> {
        ...
        // 4. 보석 에셋 생성 (루비, 에메랄드, 다이아몬드)
        var r1 = CreateGemAsset(gemDir, "Ruby_Lv1", "ruby_1", "루비 1단계", 1, 500, 0.60f, 0.30f);
        var r2 = CreateGemAsset(gemDir, "Ruby_Lv2", "ruby_2", "루비 2단계", 2, 1200, 0.40f, 0.40f);
        var r3 = CreateGemAsset(gemDir, "Ruby_Lv3", "ruby_3", "빛나는 루비 3단계", 3, 3000, 0.20f, 0.50f);
        r1.nextLevelGem = r2;
        r2.nextLevelGem = r3;
        EditorUtility.SetDirty(r1);
        EditorUtility.SetDirty(r2);

        var e1 = CreateGemAsset(gemDir, "Emerald_Lv1", "emerald_1", "에메랄드 1단계", 1, 800, 0.55f, 0.30f);
        var e2 = CreateGemAsset(gemDir, "Emerald_Lv2", "emerald_2", "에메랄드 2단계", 2, 2000, 0.35f, 0.40f);
        var e3 = CreateGemAsset(gemDir, "Emerald_Lv3", "emerald_3", "영롱한 에메랄드 3단계", 3, 5000, 0.15f, 0.55f);
        e1.nextLevelGem = e2;
        e2.nextLevelGem = e3;
        EditorUtility.SetDirty(e1);
        EditorUtility.SetDirty(e2);

        var dm1 = CreateGemAsset(gemDir, "Diamond_Lv1", "diamond_1", "다이아몬드 1단계", 1, 2000, 0.50f, 0.30f);
        var dm2 = CreateGemAsset(gemDir, "Diamond_Lv2", "diamond_2", "다이아몬드 2단계", 2, 5000, 0.30f, 0.40f);
        var dm3 = CreateGemAsset(gemDir, "Diamond_Lv3", "diamond_3", "찬란한 다이아몬드 3단계", 3, 12000, 0.10f, 0.60f);
    ...
    private static WeaponItemData CreateWeaponAsset(string dir, string assetName, string id, string name, string type, int grade, int baseAtk, float succ, float keep, int iron, List<ItemOption> options)
    {
        ...
        asset.upgradeSuccessRate = succ;
        asset.upgradeKeepRate = keep;
    ...
    private static GemItemData CreateGemAsset(string dir, string assetName, string id, string name, int level, int sellPrice, float succ, float keep)
    {
        string path = $"{dir}/{assetName}.asset";
